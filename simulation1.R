...
if(e0==1){#majority rule (a)
  ...
  pi.Y[Vdy]<-0
}else if(e0==4){ #plurality rule (b)
  gam.D<-rnorm(p,0, sqrt(0.1/p))
  pi.Y<-rnorm(p, 0, sqrt(0.5/p)) + 2.5*gam.D
  Vdy<- sample(1:length(gam.D), floor(0.5*length(gam.D)), replace = F)
  pi.Y[Vdy]<-0
}else if(e0==5){ #balaced pleiotropy (d)
  gam.D<-rnorm(p,0, sqrt(0.1/p))
...
write.csv(re.all, file=paste('MRlocal-numerical-setting',e0,'.txt',sep=""), row.names=F)
